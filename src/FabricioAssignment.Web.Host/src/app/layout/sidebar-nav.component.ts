...
import { ContentServiceProxy, ListResultDtoOfContentListDto } from '@shared/service-proxies/service-proxies';
...
export class SideBarNavComponent extends AppComponentBase implements OnInit {
    ...
    menuItems: MenuItem[] = [
        ...
        new MenuItem(this.l('CMS'), '', 'wysiwyg', '/app/cms'),
    ...
    loadContent(): Array<MenuItem> {
        let listMenuItem = new Array<MenuItem>();
            ...
            .subscribe((result: ListResultDtoOfContentListDto) => {
                result.items.map((item) => 
                {
                    this.menuItems.splice((this.menuItems.length - 1), 0,
                        new MenuItem(this.l(item.pageName), '', '', `cms/${item.id}`))
                });
            });
        return listMenuItem;
    }
